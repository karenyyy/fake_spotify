<head>
    ...
    <link rel="stylesheet" href="assets_/css/navmenu/styles.css">
    <link rel="stylesheet" href="assets_/css/portfolio.jquery.css">
    <link rel="stylesheet" href="assets_/css/fonticons.css">
    <link rel="stylesheet" href="assets_/css/style_.css">
    <link rel="stylesheet" href="assets_/fonts/stylesheet.css">
    <link rel="stylesheet" href="assets_/css/font-awesome.min.css">
    <link rel="stylesheet" href="assets_/css/bootstrap.min.css">
    <!--        <link rel="stylesheet" href="assets_/css/bootstrap-theme.min.css">-->
    ...
    <link rel="stylesheet" href="assets_/css/responsive.css" />

    <script src="assets_/js/vendor/modernizr-2.8.3-respond-1.4.2.min.js"></script>
</head>
        ...
        <div class="nav-top clearfix">
            ...
            <div class="head_top_social pull-right" style="margin: 50px">
                <ul class="list-inline">
                    <li><a href=""><i class="fa fa-skype"></i>placeholder</a></li>
                    <li><a href=""><i class="fa fa-phone"></i>placeholder</a></li>
                    <li class="top_socail">
    ...
    <div class="main-nav navbar-collapse collapse">
        <div class="container">
            <ul class="nav nav-justified">
                ...
                <li><a href="#service">NEW HITS</a></li>
                <li><a href="#lessons">TRENDING NOW</a></li>
                <li><a href="#portfolio">TOP ARTISTS 2018</a></li>
                <li><a href="#bar">GENRES</a></li>
...
<section id="service" class="service">
    ...
    <div class="_container" style="margin-top: -200px">
        <?php
        $string = file_get_contents("assets/json_files/scraped_results.json");
        $scraped_results = json_decode($string, true);

        echo "
        <div id=\"mainContent\" style=\"margin: 250px ; width: 80%\" >
       
        <h2 style=\"text-align: center; \">New hits of the week</h2><br>";

        foreach ($scraped_results as $item) {

            echo "<div class='gridViewItem'>
					<span id=\"hover\" role='link' tabindex='0' onclick='window.open(\"$item[1]\")'>
                        <div class=\"ball\" style=\"box-shadow: inset 0px 0px 0px; background-image: url('$item[2]'); width: 210px; height: 210px;\"></div>
                        <div class=\"ball-shadow\"></div>
						<div class='gridViewInfo'>
                              <p class='text' style=\" font-size: 17px; font-weight: 800\">" ."<br><br><br><br>". explode("-", $item[0])[0]."<br>" .explode("-", $item[0])[1]. "</p>
						</div>
					</span>
...
<section id="lessons" class="lessons">
        <?php
        $string = file_get_contents("assets/json_files/vevo_trending.json");
        $scraped_results = json_decode($string, true);
        ...
        foreach ($scraped_results as $item) {
            ...
            echo "<div class='gridViewItem'>
					<span id=\"hover\" role='link' tabindex='0' onclick='window.open(\"$item[2]\")'>
                        <div class='gridViewInfo'>
...
<script>
    $(document).ready(function() {
        $.getJSON("http://ws.audioscrobbler.com/2.0/?method=chart.gettopartists&limit=100&api_key=e4352f4de078644ba4e562e03f3b23d3&format=json&callback=?", function(json) {
            var html = "<h2 style=\"text-align: center;\">Top 100 Artists of the Year</h2><br>";
...
<script>
    $(document).ready(function() {
        $.getJSON("http://ws.audioscrobbler.com/2.0/?method=chart.gettoptracks&limit=24&api_key=e4352f4de078644ba4e562e03f3b23d3&format=json&callback=?", function(json) {
            var html = "<h1 style='text-align: center'>GENRES</h1><br><div id=\"mainContent\" style='margin-left: 250px'>";
            html += "<div class=\"gridViewContainer\">";
            var image_path;
            $.each(json.tracks.track, function(i, item) {
                html += "<div class=\"gridViewItem\">";
                html +="<br><br><br>";
                html += "<form id=\"similarForm\" action=\"API/similarTracks.php\" method=\"POST\">";
                html +="<input id='btn' type='submit' style='color: black; margin: 3px 25px; font-weight: bold' onclick=\"window.location.href='similarTracks.php'\" name='recommend' value='" + item.name + "'></form>";
                html += "<span role=\"link\" tabindex=\"0\" onclick=window.open(\"" +item.url +"\")>";
                image_path=JSON.stringify(item.image[3]).replace("#", "").split(",")[0].split("\":\"")[1].replace("\"","");
                html += "<br><br>"+"<div class=\"ball\" style=\"box-shadow: inset 0px 0px 0px; background-image: url("+image_path+"); width: 210px; height: 210px;\"></div>";
                html += "<p class='text'>" +"<br><br><br></p>";
                html += "<div class='gridViewInfo' style='style=\"font-size:15px; font-weight: bold; margin: 0 0 30px; font-weight: bold'>"+"<br>"+"</div></span></div>";
...
<section id="blog" class="news">
   <div id="navigation">
    ...
    <div id="tabs-pop" class="pop_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_pop.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
        ...
        foreach ($scraped_results as $item) {
            echo "<div class='gridViewItem'>
					<span role='link' tabindex='0' onclick='window.open(\"$item[3]\")'>
						<div class='gridViewInfo'>
    ...
    <div id="tabs-hip-hop" class="hip-hop_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_hip-hop.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
        ...
        foreach ($scraped_results as $item) {
            echo "<div class='gridViewItem'>
					<span id=\"hover\" role='link' tabindex='0' onclick='window.open(\"$item[3]\")'>
                        <div class='gridViewInfo'>
                            <p class='text' style=\" font-size: 17px; font-weight: 800; margin-left: -10px\">" . $item[0]."<br>" .$item[1]. "</p>
						</div>
						<img class='image' src='$item[2]'>
    ...
    <div id="tabs-electronic" class="electronic_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_electronic.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
    ...
    <div id="tabs-country" class="country_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_country.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
    ...
    <div id="tabs-rock" class="rock_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_rock.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
    ...
    <div id="tabs-rb" class="rb_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_rbsoul.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
    ...
    <div id="tabs-latino" class="latino_container">
        <?php
        $string = file_get_contents("assets/json_files/vevo_latino.json");
        $scraped_results = json_decode($string, true);
        echo "<form id=\"similarForm\" action=\"API/similarArtists.php\" method=\"POST\">
        <div id=\"mainContent\" style=\"margin: 250px; width: 80%\" >
...
<script src="assets_/js/vendor/jquery-1.11.2.min.js"></script>
<script src="assets_/js/vendor/bootstrap.min.js"></script>

<script src="assets_/js/jquery.easypiechart.min.js"></script>
<script src="assets_/js/portfolio.jquery.js"></script>
<script src="assets_/js/jquery.mixitup.min.js"></script>
<script src="assets_/js/jquery.easing.1.3.js"></script>
<script src="assets_/js/jquery.slicknav.min.js"></script>
<!--This is link only for gmaps-->
<script src="http://maps.google.com/maps/api/js"></script>
<script src="assets_/js/gmaps.min.js"></script>
<script>
    var map = new GMaps({
        el: '.ourmaps',
        scrollwheel: false,
...
<script src="assets_/js/plugins.js"></script>
